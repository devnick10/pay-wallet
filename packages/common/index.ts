...
const signInSchema = z.object({
    ...
    phone: z.string().min(10,{message:"Invalied length on number"}).max(10,{message:"Invalied length on number"}),
    password: z.string().min(3,{message:"Password should be least 4 character"}),
})
...
const onRampValidationSchema = z.object({
    ...
    amount: z.string({message:"Amount required"}).min(2,{message:"Provied valid minimum amount"}),
})
